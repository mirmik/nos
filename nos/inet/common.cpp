...
ssize_t nos::inet::datagramm_socket::recvfrom(char* data, size_t maxsize, nos::inet::netaddr* inaddr)
{
	...
	if (inaddr) *inaddr = nos::inet::netaddr(ntohl(si_other.sin_addr.s_addr), ntohs(si_other.sin_port));
	return ret;
}

nos::inet::udp_socket::udp_socket() : datagramm_socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP) {}

nos::inet::udp_socket::udp_socket(nos::inet::hostaddr addr, uint16_t port) : udp_socket()
{
	bind(addr, port);
}
...
nos::inet::rdm_socket::rdm_socket(nos::inet::hostaddr addr, uint16_t port) : rdm_socket()
{
